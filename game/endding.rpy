...
label ending_romantic_path(char_id):
    ...
    menu:
        "1. 응, 좋아해 (고백한다)":
            ...
            a "{cps=[text_speed]}정말? 그럼...우리{/cps}" # [수정 완료] 태그 파편 제거
            user "{cps=[text_speed]}사귀자{/cps}"
            show expression char_id + " love"
            a "{cps=[text_speed]}...좋아{/cps}" # [수정 완료] 태그 파편 제거
            
            # 해피 엔딩 연출
            scene bg sunset with dissolve # 해가 내려가는 하늘 (배경이름 'bg sunset' 가정)
            a "{cps=[text_speed]}해가 내려가고 하늘이 파랗게 물든 시간{/cps}" # [수정 완료] 태그 파편 제거
            a "{cps=[text_speed]}([a.name]는 [player_name]에게 팔짱을 낀다.){/cps}" # [수정 완료] 태그 파편 제거
            a "{cps=[text_speed]}너랑 이런 사이가 되어서.. 정말 좋아해{/cps}" # [수정 완료] 태그 파편 제거
        ...
        "2. (AI 정체 언급 - 동공 작아짐)":
            ...
            a "{cps=[text_speed]}뭐?(동공 작아짐){/cps}" # [수정 완료] 태그 파편 제거
