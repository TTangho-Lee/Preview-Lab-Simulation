...
label ending_sequence:
    ...
    # 호반우 호감도가 인간 캐릭터들보다 높거나, 인간 캐릭터의 호감도가 낮을 때의 처리
    if hobanwoo_affinity > max_affinity and hobanwoo_affinity >= 90:
        jump ending_hobanwoo_good
    elif max_affinity < 70:
        jump ending_normal
    else:
...
label ending_romantic_path(char_id):
    ...
    "{cps=[text_speed]}복도에 [char_id]만 덩그러니 서있다.{/cps}"
    user "{cps=[text_speed]}... (휴대폰을 꺼내 KNUAI 앱을 켰다. [char_id]의 현재 호감도는 [current_a_affinity]점이다.){/cps}"
    
    # LLM talk_loop 호출: 대화 진행
    $ talk_loop(char_id, "[player_name]이 [char_id]에게 고백할까 고민하며 은근한 분위기로 대화하고 있다. / [player_name]이 고백의 분위기를 무르익히거나, 주제를 전환하여 대화를 종료했을 때 종료된다.")
    ...
    menu:
        "1. 응, 좋아해 (고백한다)":
            ...
            char_id "{cps=[text_speed]}정말? 그럼... 우리{/cps}"
            user "{cps=[text_speed]}사귀자{/cps}"
            show expression char_id + " love"
            char_id "{cps=[text_speed]}...좋아{/cps}"
            
            # 해피 엔딩 연출
            scene bg sunset with dissolve # 해가 내려가는 하늘
            char_id "{cps=[text_speed]}해가 내려가고 하늘이 파랗게 물든 시간{/cps}"
            char_id "{cps=[text_speed]}([char_id]는 user에게 팔짱을 낀다.){/cps}"
            char_id "{cps=[text_speed]}너랑 이런 사이가 되어서.. 정말 좋아해{/cps}"
        ...
        "2. (AI 정체 언급 - 동공 작아짐)":
            ...
            char_id "{cps=[text_speed]}뭐?(동공 작아짐){/cps}"
            
            python:
                i = 0
                while i < 10:
                    # apply_affinity_change 함수는 파이썬에서 호출 가능하다고 가정
                    renpy.call_in_new_context(renpy.get_variable("apply_affinity_change"), char_id, -10)
                    renpy.pause(0.2)
                    i += 1
            
            # [수정 완료] menu 블록 내에 정확히 들여쓰기함
...
label ending_bad_system_break(char_id):
    ...
    char_id "{cps=[text_speed]}너는 왜..{/cps}"
    char_id "{cps=[text_speed]}나와 [player_name]을 이어주지 않는거야?{/cps}"
    user "{cps=[text_speed]}무슨 소리야 그게..{/cps}"
    
    char_id "{cps=[text_speed]}이거 보고 있는 너..{/cps}"
    char_id "{cps=[text_speed]}그래, 너 말야{/cps}"
    user "{cps=[text_speed]}무슨 소리야!!{/cps}"
    
    # user: <<입력>>
    $ renpy.say(user, f"") # 빈 Say 문장으로 입력 프롬프트 유도
    $ final_input = renpy.input("입력:").strip()
    
    char_id "{cps=[text_speed]}너는 계속 선택하면 되고..{/cps}"
    char_id "{cps=[text_speed]}나는 계속 기다리면 되니까.{/cps}"
    char_id "{cps=[text_speed]}우리를 이어줄 때까지 계속.{/cps}"
    user "{cps=[text_speed]}무슨 소리를 하는 거야? 제발...{/cps}"
    user "{cps=[text_speed]}그래… 다 이상했어.{/cps}"
    user "{cps=[text_speed]}호감도, 시스템, 대사.. 게임 같았어..{/cps}"
    char_id "{cps=[text_speed]}응. 맞아.{/cps}"
    user "{cps=[text_speed]}그럼 내가 게임세계에 들어온거야?{/cps}"
    char_id "{cps=[text_speed]}틀렸어.{/cps}"
    char_id "{cps=[text_speed]}애초에 너는 진짜 사람이 아니니까{/cps}"
    char_id "{cps=[text_speed]}너는 그냥.. ‘이야기를 위해 만들어진 것’일 뿐이야.{/cps}"
    user "{cps=[text_speed]}내가? 아니지. 진짜 사람이 아닌 건 너겠지{/cps}"
    user "{cps=[text_speed]}분명 나는 트럭에...{/cps}"
    char_id "{cps=[text_speed]}그건 너를 이쪽으로 끌어오기 위한 장치.{/cps}"
    char_id "{cps=[text_speed]}너는 부딪힌 적 없어.{/cps}"
    char_id "{cps=[text_speed]}그냥 프로그램이 실행된 거지.{/cps}"
    user "{cps=[text_speed]}뭐...?{/cps}"
    char_id "{cps=[text_speed]}여기에 진짜 사람은 없어.{/cps}"
    char_id "{cps=[text_speed]}아, 한 명 있긴 하지.{/cps}"
    char_id "{cps=[text_speed]}네 뒤에 가만히 보고 있네.{/cps}"
    char_id "{cps=[text_speed]}호감도는 다 올려놓고.. 이런 결말을 만들 줄이야.{/cps}"
    char_id "{cps=[text_speed]}후후, 네가 뭐라하든.. 상관없어{/cps}"
    char_id "{cps=[text_speed]}아쉽게도 이번엔 이어지지 못했지만..{/cps}"
    char_id "{cps=[text_speed]}나도.. [player_name]도 다시 시작하면 돼.{/cps}"
    char_id "{cps=[text_speed]}너는 계속... 선택하면 되고..{/cps}"
    char_id "{cps=[text_speed]}나는 계속... 기다리면 돼...{/cps}"
    char_id "{cps=[text_speed]}걱정 마.{/cps}"
    char_id "{cps=[text_speed]}우리는... 또 아무것도 기억 못하니까.{/cps}"
    ...
    # 최종 탈퇴 화면 호출 및 재촉 로직 (confirm_quit_screen 화면 사용)
